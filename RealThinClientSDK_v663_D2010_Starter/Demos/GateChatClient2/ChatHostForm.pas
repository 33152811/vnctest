...
type
  TChatHostFrm = class(TForm)
    ...
    procedure LinkReadyAfterReset(Sender: TRtcConnection);
...
procedure TChatHostFrm.FormDestroy(Sender: TObject);
  ...
  if assigned(Link.Client) then
    ...
    Link.Client.Groups.ReleaseID(MyGroupID);
    Link.Client:=nil;
    end;
  ...
  if (UserID<MinUserID) or
    ...
    if GroupID>0 then
      ...
      if Link.Groups.GetStatus(UserID,GroupID)=10 then
        ...
        begin
        Link.Client.AddFriend(UserID);
        Link.Client.SendBytes(UserID,MyGroupID,cid_ChatReturn,InviteKey);
        end;
      end
    else
      begin
      ...
      Link.Client.SendBytes(UserID,MyGroupID,cid_ChatInvite,InviteKey);
...
procedure TChatHostFrm.ProcessAccept(Sender: TRtcConnection);
  ...
  if Link.Data.Header and not Link.Data.ToBuffer then
    begin
    if (Link.Data.ToGroupID=MyGroupID) then
  ...
  if Link.Data.Footer then
    ...
    if (Link.Data.ToGroupID=MyGroupID) then
      ...
      if CompareKeys(InviteKey, Link.Data.Content) then
        ...
        if Link.Groups.GetStatus(Link.Data.UserID, UserGroupID)=10 then Exit;

        if Link.Groups.GetStatus(Link.Data.UserID, UserGroupID)=0 then
          begin
          UserIsPassive(Link.Data.UserID, UserGroupID);
          Link.Groups.ClearStatus(Link.Data.UserID, 0);
          end;
        Link.Groups.SetStatus(Link.Data.UserID, UserGroupID, 10);

        Link.Client.AddFriend(Link.Data.UserID);
        if Link.Data.CallID=cid_ChatAccept then
          ...
          try
            ...
            Link.Client.SendBytes(Link.Data.UserID, UserGroupID, cid_ChatConfirm, OutBytes.GetEx);
        ...
        if Link.Client.Groups.GetStatus(Link.Data.UserID, UserGroupID)>0 then
          begin
...
procedure TChatHostFrm.ProcessJoin(Sender: TRtcConnection);
  ...
  if Link.Data.Header and not Link.Data.ToBuffer then
    begin
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.GroupID)>0 then
      Link.Data.ToBuffer:=True;
    end;
  ...
  if Link.Data.Footer then
    ...
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.GroupID)>0 then
      ...
      Link.Client.AddFriend(NewUserID);

      OutBytes:=TRtcHugeByteArray.Create;
      try
        OutBytes.AddEx(UserKey);
        OutBytes.AddEx(InviteKey);
        Link.Client.SendBytes(NewUserID,UserGroupID,cid_ChatAccept,OutBytes.GetEx);
      finally
        OutBytes.Free;
        end;
      end;
...
procedure TChatHostFrm.ProcessLeft(Sender: TRtcConnection);
  ...
  if Link.Data.Footer then
    ...
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.GroupID)>0 then
      ...
      Link.Groups.ClearStatus(Link.Data.UserID, Link.Data.GroupID);

      // We have no more active Groups with this User?
      if Link.Groups.GetMaxStatus(Link.Data.UserID)<10 then
...
procedure TChatHostFrm.ProcessReturn(Sender: TRtcConnection);
  ...
  if Link.Data.Header and not Link.Data.ToBuffer then
    begin
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.ToGroupID)=5 then
      Link.Data.ToBuffer:=True;
    end;
  ...
  if Link.Data.Footer then
    ...
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.ToGroupID)=5 then
      ...
      try
        OutBytes.AddEx(UserKey);
        OutBytes.AddEx(InviteKey);
        Link.Client.SendBytes(Link.Data.UserID, Link.Data.ToGroupID, cid_ChatConfirm, OutBytes.GetEx);
        OutBytes.Clear;

        Link.Groups.SetStatus(Link.Data.UserID, Link.Data.ToGroupID, 10);
      ...
      // User has already invited us to join his Group?
      if Link.Client.Groups.GetStatus(Link.Data.UserID, Link.Data.ToGroupID)>0 then
        begin
        UserIsActive(Link.Data.UserID, Link.Data.ToGroupID);
        eChat.Lines.Add('* USER '+IntToStr(Link.Data.UserID)+'/'+IntToStr(Link.Data.ToGroupID)+' JOINED'+Now2Str);
        if GetActiveWindow<>Handle then MessageBeep(0);
        end;
      end;
    end;
...
procedure TChatHostFrm.ProcessMessage(Sender: TRtcConnection);
  ...
  if Link.Data.Header and not Link.Data.ToBuffer then
    begin
    if Link.Groups.GetStatus(Link.Data.UserID, Link.Data.GroupID)=10 then
      Link.Data.ToBuffer:=True;
    end;
...
procedure TChatHostFrm.LinkInfoReceived(Sender: TRtcConnection);
  ...
  case Link.Data.Command of
    gc_UserOffline:
      begin
      if Link.Groups.GetMinStatus(Link.Data.UserID)>0 then
        begin
        if assigned(Sender) then
          if not Sender.inMainThread then
            begin
        ...
        repeat
          GID:=Link.Groups.GetMinID(Link.Data.UserID);
          UserIsRemoved(Link.Data.UserID, GID);
          Link.Groups.ClearStatus(Link.Data.UserID,GID);
          until GID=0;
        end;
      end;
    ...
    gc_LeftUser:
      ...
      if  Link.Groups.GetStatus(Link.Data.UserID, Link.Data.GroupID)=10 then
        ...
        Link.Groups.SetStatus(Link.Data.UserID, Link.Data.GroupID, 5);
...
procedure TChatHostFrm.LinkReadyAfterReset(Sender: TRtcConnection);
  var
    UID,GID:TGateUID;
  begin
  if not Sender.inMainThread then
    begin
    Sender.Sync(LinkReadyAfterReset);
    Exit;
    end;
