    ...
    if FUserListNew and GCM.Client.Ready then
      ...
      while p<=l do
        ...
        if fs[p] in ['0'..'9'] then
          ...
          GCM.Client.SendBytes(aUser,0,cid_AddMeFriend);
          Inc(cnt);
          ts:='';
          end;
        Inc(p);
...
procedure TGateClientForm.DataReceived(Sender: TRtcConnection);
  ...
  else if GCM.Data.CallID=cid_AddMeFriend then
    begin
    if GCM.Data.Header then // Start Buffering content after we receive the header
      GCM.Data.ToBuffer:=True;
    if GCM.Data.Footer then // When the footer is here, we have the complete package
      begin
      ...
      GCM.Client.RemoveFriend(GCM.Data.UserID);
      GCM.Client.AddFriend(GCM.Data.UserID);
      end;
    end;
  end;
...
procedure TGateClientForm.InfoReceived(Sender: TRtcConnection);
  ...
  case GCM.Data.Command of
    gc_BeFriend:  // Received "FriendAdd" notification from the user.
                  // Now, we can add the user to our receiver Group ...
                  GCM.Client.AddUserToGroup(GCM.Client.MyUID, 1, GCM.Data.UserID);
